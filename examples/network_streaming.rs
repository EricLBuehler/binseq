...
use binseq::{BinseqHeader, Policy, Result, StreamReader, StreamWriterBuilder};
...
fn client() -> Result<()> {
    ...
    println!("Client: Received header with sequence length = {}", header.slen);
    
    // Read records as they arrive
    let mut count = 0;
    while let Some(record) = reader.next_record()? {
        println!("Client: Received record {} with flag = {}", count, record.flag());
        count += 1;
    }
